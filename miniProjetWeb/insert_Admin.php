...
$box =isset($_REQUEST["check"])?$_REQUEST["passs"]:NULL;

try{
  if(($lo!=NULL)&&($pas!=NULL)&& $box==NULL ){
        //---------------------------------------
	   ...
	   $stmt = $pdo->prepare("insert into utilisateur (idUtil,pass,droits) values (:log,:pass,'admin')");
		 $hash =  password_hash($pas,PASSWORD_DEFAULT);
		 $stmt->bindParam(':pass',$hash); 
		 $stmt->bindParam(':log',$lo); 
		 $stmt->execute();
